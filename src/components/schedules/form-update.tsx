...
import { ScheduleWithRelations, updateSchedule } from "@/actions/schedules";
...
export function FormUpdateSchedule({ schedule, categories, types }: { schedule: ScheduleWithRelations } & FormProps) {
    const [loading, setLoading] = useState(false);
    const [date, setDate] = useState<Date>(schedule.remindAt);
    const [time, setTime] = useState(schedule.remindAt.toISOString().slice(11, 16));
    const [message, setMessage] = useState<{ type: "success" | "error"; text: string } | null>(null);
    // Controlled select values for type and category
    const [selectedType, setSelectedType] = useState<string>(schedule.type?.code ?? "");
    const [selectedCategory, setSelectedCategory] = useState<string>(schedule.category?.code ?? "");
    ...
    const handleSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
        ...
        const title = (formData.get("title") as string) || schedule.title;
        const description = (formData.get("description") as string) || schedule.description;
        const typeCode = selectedType || schedule.type?.code || "";
        const categoryCode = selectedCategory || schedule.category?.code || "";
        ...
        try {
            ...
            if (res.success) {
                ...
                setDate(schedule.remindAt);
                setTime(schedule.remindAt.toISOString().slice(11, 16));
            }
        } catch (error: any) {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
                ...
                <Input name="title" placeholder="Contoh: Minum Vitamin / Daily Standup" defaultValue={schedule.title} required />
            </div>

            <div className="space-y-1">
                <label className="text-xs font-semibold text-zinc-600 dark:text-zinc-400">Deskripsi (Opsional)</label>
                <Textarea name="description" placeholder="Tulis catatan tambahan di sini..." defaultValue={schedule.description ?? ""} rows={3} />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                    <Select name="typeCode" value={selectedType} onValueChange={setSelectedType} required>
                ...
                <div className="space-y-1">
                    <Select name="categoryCode" value={selectedCategory} onValueChange={setSelectedCategory} required>
                        <SelectTrigger className="w-full">
